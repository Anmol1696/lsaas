...
RESP=$($CHAIN_BIN tx wasm store "$DIR/hackatom.wasm"  --keyring-backend test \
  --from val1 --gas auto --fees $FEES -y --chain-id $CHAIN_ID -b block -o json --gas-adjustment 1.5)
echo "$RESP"
...
$CHAIN_BIN tx wasm set-contract-admin "$CONTRACT" "$($CHAIN_BIN keys show test1 -a --keyring-backend test)" \
  --from val1 --keyring-backend test --gas-adjustment 1.5 --gas "auto" --fees "$FEES" -y --chain-id $CHAIN_ID -b block -o json | jq
...
RESP=$($CHAIN_BIN tx wasm store "$DIR/burner.wasm" --gas-adjustment 1.5 --fees "$FEES" \
  --from val1 --gas "auto" -y --chain-id $CHAIN_ID --keyring-backend test -b block -o json)
...
$CHAIN_BIN tx wasm migrate "$CONTRACT" "$BURNER_CODE_ID" "{\"payout\": \"$DEST_ACCOUNT\"}" --from test1 \
  --chain-id $CHAIN_ID --gas "auto" --keyring-backend test --gas-adjustment 1.5 --fees "$FEES" -b block -y -o json | jq
